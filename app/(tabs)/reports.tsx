...
import {
  ...
  generateCardFeeCalculationSection,
  generateWeeklyTotalsSection,
...
export default function ReportsScreen() {
  const { generateNightlyReport, clearNightlyReport, generateAggregatedReport, getHistoricalReports, orders, settings } = usePOS();
  ...
  const generateReportText = useCallback(async (report: NightlyReport) => {
    ...
    const cardFeeCalculation = generateCardFeeCalculationSection(report, isTrainingReport || false, formatCurrency);
    
    // Check if today is Thursday for weekly totals
    const today = new Date();
    const isThursday = today.getDay() === 4; // 4 = Thursday
    
    let weeklyTotals = '';
    if (isThursday) {
      // Get historical reports for weekly calculation
      const historicalReports = await getHistoricalReports();
      weeklyTotals = generateWeeklyTotalsSection(report, isTrainingReport || false, formatCurrency, historicalReports);
    }
...
${cardFeeCalculation}`;
    ...
    // Add weekly totals only on Thursday
    if (isThursday) {
      reportText += `\n\n${weeklyTotals}`;
    }
